      ...
      <div className="max-w-4xl mx-auto text-center">
        <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold mb-6 leading-tight">
          ...
          {prefersReducedMotion ? (
            <span className="text-green-600 dark:text-green-500">{phrases[0]}</span>
          ) : (
            <span ref={animatedTextRef} className="inline-block min-w-[300px] sm:min-w-[400px] text-left" />
          )}
        </h1>
...
export function useTypewriter({ ref, phrases, theme, enabled }: UseTypewriterOptions) {
  useEffect(() => {
